...
interface BreatheTextProps {
  text: string;
  className?: string;
}

export function BreatheText({ text, className }: BreatheTextProps) {
  return (
    <div
      className={cn(
        "flex items-center justify-center overflow-hidden py-10",
        className,
      )}
    >
      <span className="animate-breathe text-6xl md:text-9xl text-primary tracking-tighter">
        {text}
      </span>
    </div>
  );
