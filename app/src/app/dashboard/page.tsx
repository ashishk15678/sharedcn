        ...
        <div className="w-full max-w-5xl mx-auto p-4 md:p-8">
            ...
            <div className="flex-1 min-w-0 border border-zinc-200 rounded-xl bg-white p-6 flex flex-col h-full overflow-y-auto">
              {!selectedComponent ? (
                ...
                <>
                  ...
                  {/* Metrics summary */}
                  {selectedComponent &&
                  Array.isArray(selectedComponent.metrics) &&
                  selectedComponent.metrics.length > 0 ? (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                      {(() => {
                        const m = selectedComponent.metrics[0] || {};
                        return [
                          {
                            label: "Installs",
                            value: m.installations ?? 0,
                            icon: (
                          ...
                          },
                          {
                            ...
                            value: m.uploads ?? 0,
                            icon: (
                              <FileBoxIcon className="w-5 h-5 text-blue-500" />
                            ),
                          ...
                          <div
                            key={stat.label}
                            className={`flex flex-col items-center justify-center rounded-xl border p-3 ${stat.color}`}
                          >
                  ...
                  {/* File Explorer + Code Viewer + Live Preview */}
                  {selectedComponent &&
                    selectedComponent.files &&
                    selectedComponent.files.length > 0 && (
                      <div className="flex flex-col md:flex-row gap-4 mt-4">
                        {/* File Explorer */}
                        <div
                          className="w-full md:w-48 bg-white/60 border border-zinc-200 rounded-xl shadow-sm p-2 flex flex-col gap-1"
                          style={{ backdropFilter: "blur(8px)" }}
                        >
                          <div className="font-semibold text-zinc-700 mb-2 text-xs uppercase tracking-wider">
                            Files
                          </div>
                          {selectedComponent.files.map((file: any) => (
                            <button
                              key={file.filename}
                              ...
                              className={`w-full text-left px-3 py-2 rounded-lg text-sm font-mono transition-all ${
                                ...
                                (!activeFile &&
                                  file.filename === selectedComponent.mainFile)
                                  ? "bg-gradient-to-r from-yellow-100 to-yellow-50 text-yellow-900 font-bold border border-yellow-200"
                                  : "hover:bg-zinc-100 text-zinc-700"
                              } ${
                                file.filename === selectedComponent.mainFile
                        ...
                        <div className="flex-1 flex flex-col gap-4">
                          {/* Code Viewer */}
                          <div
                            className="bg-white/80 border border-zinc-200 rounded-xl shadow-inner p-4 font-mono text-xs text-zinc-800 min-h-[200px] max-h-[350px] overflow-auto"
                            style={{ backdropFilter: "blur(8px)" }}
                          >
                            <pre>
                              {(() => {
                                const file = selectedComponent.files.find(
                                  (f: any) =>
                                    f.filename ===
                                    (activeFile || selectedComponent.mainFile)
                                );
                                return file ? file.code : "// No file selected";
                              })()}
                            </pre>
                          </div>
                          {/* Live Preview (Babel-powered, placeholder for now) */}
                          <div
                            className="bg-gradient-to-br from-white/80 to-yellow-50 border border-yellow-100 rounded-xl shadow p-4 min-h-[120px] flex items-center justify-center text-zinc-400 italic"
                            style={{ backdropFilter: "blur(8px)" }}
                          >
                            {/* TODO: Inject and render main file's default export using Babel */}
          ...
          <div className="flex flex-row w-full items-center space-x-4">
            ...
            w-full bg-zinc-800 rounded-md p-2"
                  ...
                  {/* Replace the single-file creation UI with a VSCode-like multi-file editor */}
                  <div className="flex flex-col  justify-center gap-2">
                    <p className="text-zinc-400">Files: *</p>
                    {/* VSCode-like file tab bar */}
                    <div className="flex flex-row items-center gap-1 mb-2 border-b border-zinc-700 overflow-x-auto">
                      {comp.code.map((file, idx) => (
                        ...
                        >
                          ...
                          <input
                            type="radio"
                            name="mainFile"
                            checked={comp.code[idx].filename === comp.mainFile}
                            onChange={() =>
                              setComp({
                                ...comp,
                                mainFile: comp.code[idx].filename,
                              })
                            }
                      ...
                      <button
                        ...
                        onClick={() => {
                          let base = "File" + (comp.code.length + 1);
                          let ext = ".ts";
                          ...
                          while (
                            comp.code.some((f) => f.filename === filename)
                          ) {
                            filename = base + i + ext;
                            i++;
                          }
                    ...
                    <div className="flex-1 min-h-[350px] max-h-[60vh] border border-zinc-800 rounded-b-xl bg-zinc-900 overflow-hidden shadow-inner">
                      <MonacoEditor
                        height="350px"
                        defaultLanguage={
                          comp.code[activeFileIdx]?.filename.endsWith(".tsx")
                            ? "typescript"
                            : comp.code[activeFileIdx]?.filename.endsWith(".ts")
                            ? "typescript"
                            : comp.code[activeFileIdx]?.filename.endsWith(
                                ".jsx"
                              )
                            ? "javascript"
                            : comp.code[activeFileIdx]?.filename.endsWith(".js")
                            ? "javascript"
                            : "plaintext"
                        }
                        language={
                    ...
                    {/* File type validation and main file selection error */}
                    {comp.code.some(
                      (f) =>
                        ![".js", ".ts", ".jsx", ".tsx", ".css"].some((ext) =>
                          f.filename.endsWith(ext)
                        )
                    ) && (
                      <div className="text-red-500 text-xs mt-2">
                        Only js, ts, jsx, tsx, css files allowed.
                      </div>
                    )}
                    {!comp.mainFile && (
                      <div className="text-red-500 text-xs mt-2">
                        Please select a main file.
                      </div>
                    )}
                  </div>
                </div>
